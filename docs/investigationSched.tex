...
In this case, to ease the implementation, we limit the maximum allowance of shifting containers.
...
To reduce the amount of unused resources, the worst case tim programmed for a task will be obtained based pn probability distribution and allowed risk.
In such case the tasks may exceed the configured worst case run-time, but the probability is low and contemporaneity factor even lower.
We obtain run-time values and probability distributions through experimental measurements. 
Those measurements give us an average and peak value. The closer we set the worst case execution time to the average, the higher is the risk of exceeding it.
