...
%The overhead of the virtual internal network could push on the actual performance. 
%We should check if there is a way of reading io of the network without having to create a virtual subnet.
Also, possible might be to read disk I/O of a container, even though it could be that it is not necessary. 
%TODO: NOTE Is there an Icinga plugin made for docker containers already available that can be adapted?
%
%% In the program
%
%% Considerations
...
%TODO: detail fifo, correct signal and int specifications
%% How
...
\begin{enumerate}
	...
	\end{enumerate}
	\item shared memory - probably the easiest type. The advantage in case of threads is that they 
...
\end{enumerate}
The easiest approach for the orchestrator's threads is to use shared variables and a semaphore/mutex. 
On the other hand, the most flexible is via sockets which will be used for the communication of orchestrator and monitoring plugin.
...
%TODO: socket implementation description
Implementation details for sockets have still to be investigated.
...
Thread two has to scan the PID list anyway.on the other hand
...
%TODO: socket implementation description
Considerations for a socket implementation have still to be taken.
