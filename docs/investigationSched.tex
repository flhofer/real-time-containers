...
We have already seen that dynamic scheduling without any additional assumptions is undecideable . % TODO; check if that has been introduced.. actually
...
In such case the tasks may exceed the configured worst case run-time $w_i$, but the probability is low and contemporaneity factor even lower.
We obtain run-time values and probability distributions through experimental measurements. 
Those measurements give us an average ($\bar c_i$) and peak value ($\max c_i = w_i$). The closer we set our new $w_i$ to the average $\bar c_i$, the higher is the risk of exceeding it.
...
While before the resource slot between average and peak is used with a low probability only, reducing the WCET increases resource utulization at a probability cost.
If for example we have nine tasks with WCET 10, period $p_i$ 50 and $\bar c_i$ 5 and probability of exceding $\bar c_i$ of $10 \exp{-10}$ we can take the following assumptions.
We can put all tasks $w_i$ to 5, resulting in a total utilzation factor $U$ of $0.9$.
The 10\% margin can be used by any of the containers to bridge the gap between average $\bar c_i$ and WCET of 5.
The risk is given by the contemporaneity factor, i.e., the probability that two or more tasks exceed their average value at the same time, and the sum of eccedence is more than 10
\% of $U$.

We have now given the following.
Getting closer to $c_{i\_max}$, the probability $\pi_i$ of a task to exceed the set $wi$ decreases according to the PDF measured for the task. 
...
Or put differently, if we set $w_i$ closer to the average than to the peak, the resources free and will not be used with a probability F.

The utilization buffer is defined thus as 
\begin{equation}
	U_B = 1 - U
\end{equation}

And the worst case utilization excedence of a task i is 
\begin{equation}
	U_{Ei} = \frac{C_{i_{max}}- w_i}{p_i}
\end{equation}

With this two utilization factors we can then compute the accomodation rate of the buffer, $c$, which is defined as

\begin{equation}
	c = 1 + \lceil \frac{U_B}{U_{Ei}} \rceil
\end{equation}
...
The risk taken is influenced by mutliple factors. If the buffer time is big enough to accommodate more than one exceeding tasks, the probability of failure reduces even more.
