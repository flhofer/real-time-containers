...
The task set remains thus schedulable with probability of failure $P$ and resource savings as long as it complies with the mentioned parameters. %  $U_S$
...
% test setup, IAAS Aws C5 and Bare metal Ubuntu Server 18.04 LTS, kernel 4.19.31-rt18
%TODO: add introductory paragraph. 
A set of tests allows us to validate the orchestrator and systems configuration.
For these tests we first prepared all running systems with new kernels and patches.
We prepared then a set of test configurations to validate different running scenarios. 
The rest of this section details steps further.

For the results in this section, we reconfigured both our AWS T3/C5 instances and the bare metal server used in \cite{Hoferetal2019} with the latest LTS operating system release, Ubuntu Server $18.04.2$.
In addition, we upgraded the kernel of all systems to their latest available version close or equal to $4.19.31$, and patched it with the latest \texttt{PREEMPT-RT} release, $4.19.31-rt18$.
...
In this configuration, we disabled some debug features and increased the scheduler base tick frequency.
...
In addition, it can reclaim the unused bandwidth of EDF tasks and reallocate it to other EDF processes, permitting them to continue over their WCET without throttling.
...
The selected test batch consists of a set of preliminary boundary tests and set of simulation tests.
...
The second configured group includes two containers with $1.9ms$ run-time $t_i$, $2.5ms$ WCET and $5ms$ period.
...
The last test is the simulation of the parallel operation of multiple containers running the flow-control program. 
...
%TODO :scripts and containers
%TODO :scripts for tests and test-runs
