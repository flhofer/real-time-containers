...
Ultra-books and laptops often disable Hyper-threading to save energy when operating on battery. Thus, in oder to have all threads running, plug-in the power chord before continuing. %TODO: find reference! Dell ?? 
...
%The host will continue normal operation on other tasks, such as browsing, compiling documents or streaming content. This is to verify that the Hyper-threading component has been take into account.

%% REPEAT TESTS FROM HERE
...
Thus, in oder to give the virtual machine proper computing power, isolate it from the host processes, and to allow isolation of the real-time tasks from non real-time processes, we will now set up cpu isolation for the host CPU.
...
\begin{verbatim}
	...
	cset shield --shield --threads --pid <read-pids>
\end{verbatim}
...
With isolation the average delay is in some cases almost ten times the cycle time, causing the test duration to shift from planned 100 seconds to over 16 minutes. One reason could be that load balancing is still active, thus the thread could be shifted in the host onto other vCPUs to keep the  average workload balanced. This is a phenomena particularly visible under low system load where the Complete Fair Scheduler (CFS) tries to distribute running tasks equally among the available vCPUs. As a side effect, this allows the CPU-governor to clock down the main bus (if enabled) and thus to save power.
...
%#1 might be redone. keep attn to the load on the system, might infuence the outcome
%\noindent \small \#1 \texttt{T: 0 ( 1452) P:99 I:1000 C: 100000 Min: 0 Act: 2815 Avg: 2183 Max:  867985}\\
\noindent \small \#1 \texttt{T: 0 ( 1677) P:99 I:1000 C: 100000 Min: 1 Act: 4635 Avg: 8114 Max:  991370}\\
...
\noindent \small \#4 \texttt{T: 0 ( 1598) P:99 I:1000 C: 100000 Min: 1 Act:   88 Avg: 4195 Max:  893460}

As we can see from the results, the values between with and without load balancer did not change much.
%TODO: verify
The values are more uniform, now that tasks are not moved around on the vCPUs, but the values are still very high.
On reason might be that the virtual machine has its own scheduler which might perform the same CFS distribution. We should therefore see some change if we perform the same configuration also for the guest, but first we will do a last test.

For now only the tasks, user or system, have been moved from their original CPU to vCPU 3. 
Interrupts on a multi-core system can be triggered on every active core and handled by the the specific routine. 
...
%\noindent \small \#1 \texttt{T: 0 ( 1456) P:99 I:1000 C: 100000 Min: 0 Act:  198 Avg: 1693 Max:  86277}\\
%\noindent \small \#2 \texttt{T: 0 ( 1778) P:99 I:1000 C: 100000 Min: 2 Act:  394 Avg: 7539 Max:  973262}

%% UNTIL HERE
\noindent \small \#1 \texttt{T: 0 ( 1868) P:99 I:1000 C: 100000 Min: 1 Act: 4617 Avg: 5309 Max:  854884}\\
\noindent \small \#2 \texttt{T: 0 ( 1941) P:99 I:1000 C: 100000 Min: 1 Act: 8236 Avg: 5106 Max:  828495}
\noindent \small \#3 \texttt{T: 0 ( 2063) P:99 I:1000 C: 100000 Min: 1 Act: 8929 Avg: 8509 Max:  905736}
...
One main reason could be that the last vCPU is not actually a separate core but only one of the two threads of the Hyper-threading instance of the physical core two. 
