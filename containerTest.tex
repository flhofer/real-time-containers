...
There exist three types of real-time operations: 1) soft-real-time, where the value of computation decreases as the deadline overshoot gets bigger, 2) firm-real-time, where the computation value of overshoots is zero, and 3) hard-real-time, where missed deadline is dangerous for environment and/or people.
%TODO: cite Butazzo
...
\begin{itemize}
	...
	Snap based as well as Balena based approaches are investigated on this hardware optimized image
...
%TODO: fix that phrase
CoreOS has been excluded from the candidate list as it comes pre-configured with a LXC/LXD while being stripped down to the bare minimum (LXC/LXD has been excluded beforehand).
...
Ubuntu Core is a project by Canonical intended to bring the advantage of standard Ubuntu technology to run small and transactionally upgradeable applications on embedded devices. 
The developed application for the end device is shipped as a software package, a snap, and run sand-boxed in a container-like environment, while keeping the transparency of a traditional application. 
...
An eventual cloud based host will also not run a hard-real-time capable kernel by default. Thus, configuration and tweaking tests of a similar VM host could give some more comparison and reference data.
...
A more advanced version, \texttt{stress-ng}, is also available. It features more tests and configuration options, and will be used for the duration tests in future work \cite{stress02}.
...
According to the Docker reference manual, \cite{docker03}, the init binary for the standard Docker-based installations is obtained via the \texttt{tini} project, and can thus be safely pulled from there. The git repository can be found at \cite{tini01}.
...
The low load allows the scheduler to fill up the available CPU time with other threads, and thus, also to distribute them (load balance) between the vCPUs.
This movement of tasks causes the CPU registers to be flushed and L1-L2 caches will have to be refilled from L3. 
...
Now we will restart the cores (via the hot-plug feature of the kernel) to force moving of tasks to the other cores. We will use the procedure described in \cite{lrt02}.
...
Even after changing load balanacer and IRQ affinity settings the values remain in the same range. It seems that the CPU isolation and configuration of the guest settings either has no impact or worsens the performance.
...
In the last meeting it was finally decided to continue with Docker as a main containerization platform. Furthermore, Kubernetes will be introduced as management tool for the virtualized machine and its containers.
...
For the following weeks more tests could clarify additional behavior of virtual machines in the cloud. An example is a test run on dedicated server with four or more cores. A hardware solution containing multiple NUMA nodes would enable to verify improvements through reduced cache misses.
